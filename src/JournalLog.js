...
import { getJournalEntries, replaceJournalEntry } from './components/journaling/journalDbFunctions';
...
async function saveEdits(setShowEditor, popupContent, setJournalArray) {
  ...
  replaceJournalEntry({title: popupContent.title, docID: auth.currentUser.uid + "_" + popupContent.dateTime, body: editBoxValue});
...
const journalEntryGetter = async (setJournalArray) => {
  ...
  const userJournalEntries = journalEntries.filter((entry) => entry.id.split('_')[0] === auth.currentUser.uid);
    ...
    <div className="JournalLog">
      ...
      {journalArray.length > 0 ? (
        ...
        <div className="firstBox">
          ...
          <p className="body" onClick={handleBoxClick}>
            No journal entries found. Reply to a reminder email to create a new entry.
          </p>
        </div>
      )}
      ...
      {showEditor && (
        <div className="popup">
          <textarea className="editBox" placeholder="Type..." name="editBox" id="editBox" defaultValue={popupContent.body}></textarea>
          <br />
          <button className="button" onClick={() => saveEdits(setShowEditor, popupContent, setJournalArray)}>Save</button>
